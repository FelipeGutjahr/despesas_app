...
AutoCompleteTextField autoCompleteTextFieldPortador;
...
class _NovoLancamentoPageState extends State<NovoLancamentoPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Drawer(
        child: Column(
          children: [getTopContainer(), getBodyDrawer(_width)],
        ),
      ),
    );
  }
  ...
  Widget getBodyDrawer(double _width) {
    return Expanded(
      ...
      child: Column(
        ...
        children: [getForm(_width), getBtnSalvar()],
      ),
    ));
  }
      ...
      child: Column(
        children: [
          Row(
            ...
            children: [
              Expanded(child: getFieldData()),
              SizedBox(width: 10),
              Expanded(child: getFieldValor())
            ],
          ),
          SizedBox(height: 10),
          getFieldPortador(),
  ...
  Widget getFieldPortador() {
    return Observer(builder: (_) {
      return controller.getPortadores.isEmpty
          ...
          : CustonWidget.getAutocCompleteTextFormFieldPortador(
              ...
              itemSubmitted: (item) {
                ...
                controller.getReceita
                    ? controller.lancamentoModel.planoDebito = item.plano
                    : controller.lancamentoModel.planoCredito = item.plano;
              });
    });
  }
  ...
  Widget getFieldContrapartida() {
    return Observer(builder: (_) {
      return controller.getContas.isEmpty
          ? CustonWidget.getTextFormField(
              context: context,
          ...
          : CustonWidget.getAutocCompleteTextFormFieldPlano(
              ...
              itemSubmitted: (item) {
                ...
                controller.getReceita
                    ? controller.lancamentoModel.planoCredito = item
                    : controller.lancamentoModel.planoDebito = item;
              },
  ...
  Widget getCreditoForm() {
    return Column(
      children: [
        ...
        Observer(
          builder: (_) {
            return controller.getIsCredito
                ? getCheckBoxIsParcelado()
                : Container(height: 0.0, width: 0.0);
          },
        ),
        Observer(
          builder: (_) {
            return controller.getIsParcelado
                ? getFieldParcelas()
                : Container(height: 0.0, width: 0.0);
          },
        )
      ...
      children: [
        CustonWidget.getTextFormField(
            ...
            onSaved: (value) => value.isNotEmpty
                ? controller.lancamentoModel.qtdParcelas = int.parse(value)
                : null),
        SizedBox(height: 10),
      ],
    );
      ...
      child: Column(
        children: [
          Observer(
            builder: (_) {
              return controller.getIsParcelado
                  ? Text(controller.getResultadoParcelas != null &&
                          !controller.getReceita
                      ? 'Pago em ${controller.getQtdParcelas}x de R\$${controller.getResultadoParcelas}'
                      : '')
                  : Container(height: 0.0, width: 0.0);
            },
          ),
